...
pushd boost-build
  ...
  pushd boost
    ...
    Bootstrap
    b2 headers
    b2 install --prefix=../../boost-1.60.0 --with-chrono --with-filesystem --with-date_time --with-atomic --with-thread --with-system --with-regex --with-atomic --with-program_options toolset=msvc-14.0 windows-api=store link=shared --variant=release
  popd
